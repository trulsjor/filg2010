  ...
  describe('når kamper blir oppdatert', () => {
    it('returnerer berørte turneringer i affectedTournaments', async () => {
      ...
      const mockScraperService = {
        fetchMultipleResults: vi.fn().mockResolvedValue(
          new Map([['12345', { matchId: '12345', result: '30-25' }]])
        ),
      };

      const result = await updateResults({
        fileService: mockFileService as any,
        scraperService: mockScraperService as any,
        logger: mockLogger,
        now: () => new Date('2025-01-15'),
      });
      ...
      expect(result.affectedTournaments.get('https://handball.no/turnering/456')).toBe('Regionserien Gutter 15');
    });
    ...
    it('samler unike turneringer når flere kamper oppdateres', async () => {
      ...
      const mockScraperService = {
        fetchMultipleResults: vi.fn().mockResolvedValue(
          new Map([
            ['111', { matchId: '111', result: '30-25' }],
            ['222', { matchId: '222', result: '28-22' }],
            ['333', { matchId: '333', result: '35-30' }],
          ])
        ),
      };
