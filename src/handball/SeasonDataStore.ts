...
export class SeasonDataStore {
  ...
  loadConfig(): Config {
    ...
    if (!isConfig(parsed)) {
      throw new Error(`${this.configPath} mangler currentSeason eller squads`)
    }
    return parsed
  }
  ...
  loadTables(squadId: string, slug: string): LeagueTable[] {
    ...
    return Array.isArray(parsed) ? (parsed as LeagueTable[]) : []
  }
  ...
  loadPlayerStats(squadId: string, slug: string): PlayerStatsData | null {
    const parsed = this.read(squadId, slug, 'player-stats.json')
    return isPlayerStatsData(parsed) ? parsed : null
  }
  ...
  listSeasons(): SeasonManifestEntry[] {
    ...
    for (const squadId of readDirectories(this.dataDir)) {
      const squadDir = path.join(this.dataDir, squadId)
      for (const slug of readDirectories(squadDir)) {
        const seasonPath = path.join(squadDir, slug, 'season.json')
        if (!fs.existsSync(seasonPath)) continue
        const parsed: unknown = JSON.parse(fs.readFileSync(seasonPath, 'utf-8'))
        if (!isSeasonManifestEntry(parsed)) continue
        entries.push({
...
function isPlayerStatsData(value: unknown): value is PlayerStatsData {
  ...
  return 'players' in value && Array.isArray(value.players)
}

function isSeasonManifestEntry(value: unknown): value is SeasonManifestEntry {
  if (typeof value !== 'object' || value === null) return false
  for (const key of ['squadId', 'squadName', 'seasonId', 'seasonName', 'slug', 'status']) {
