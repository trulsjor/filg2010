...
import type { Match, Team } from '../src/types/index.js'
...
test.describe('Data Parsing', () => {
  ...
  test('should handle empty values in match data', () => {
    ...
    const futureMatch = matches.find((m: Match) => m['H-B'] === '')
  ...
  test('should have matches for multiple teams', () => {
    ...
    const teams = new Set(matches.map((m: Match) => m.Lag))
...
test.describe('Metadata Parsing', () => {
  ...
  test('should match actual data counts', () => {
    ...
    const uniqueTeams = new Set(matches.map((m: Match) => m.Lag))
